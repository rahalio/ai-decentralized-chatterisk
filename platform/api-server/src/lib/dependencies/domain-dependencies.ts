...
const DOMAIN_PATH_PATTERNS: Array<{ pattern: RegExp; domain: string }> = [
  ...
  { pattern: /\/aggregates\//, domain: "aggregates" },
  { pattern: /\/alerts\//, domain: "alerts" },
  { pattern: /\/comments\//, domain: "comments" },
  { pattern: /\/dossiers\//, domain: "dossiers" },
  { pattern: /\/external-links\//, domain: "external-links" },
...
export async function injectDomainDependencies(
  ...
  if (!domainDeps) {
    ...
    switch (domain) {
      case "aggregates":
        domainDeps = buildAggregatesDomainModule(dynamoClient);
        break;
      case "alerts":
        domainDeps = buildAlertsDomainModule(dynamoClient);
        break;
      case "comments":
        domainDeps = buildCommentsDomainModule(dynamoClient);
        break;
      case "dossiers":
        domainDeps = buildDossiersDomainModule(dynamoClient);
        break;
      case "external-links":
        domainDeps = buildExternalLinksDomainModule(dynamoClient);
        break;
      case "identity":
...
export interface DomainDependencies {
  aggregates: AggregatesDomainModule;
  alerts: AlertsDomainModule;
  comments: CommentsDomainModule;
  dossiers: DossiersDomainModule;
  "external-links": ExternalLinksDomainModule;
 ...
 */
export function getAggregatesDeps(
  request: FastifyRequest,
): DomainDependencies["aggregates"] {
  return getDomainDeps(request, "aggregates");
}
...
export function getCommentsDeps(
  request: FastifyRequest,
): DomainDependencies["comments"] {
  return getDomainDeps(request, "comments");
}
...
export function getExternalLinksDeps(
  request: FastifyRequest,
): DomainDependencies["external-links"] {
  return getDomainDeps(request, "external-links");
}
